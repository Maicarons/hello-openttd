...
hello-openttd 本身是绿色便携的桌面应用，无需安装器，解压即用。
...
| 平台 | 产物 |
|------|------|
| Windows | `hello-openttd-<版本>-windows-x64.zip` |
| Linux | `hello-openttd-<版本>-linux-x64.tar.gz` / `.AppImage` |
| macOS | `hello-openttd-<版本>-macos-universal.zip` |

> Releases 页面同时提供 `sha256sums.txt` 校验文件，建议下载后核对（见[下载与镜像](./downloads#文件校验)）。

### Windows

1. 解压 zip 到任意目录（建议放在非系统盘，如 `D:\hello-openttd`）。
2. 运行 `hello_openttd.exe`。
...
若使用 AppImage：`chmod +x hello-openttd-*.AppImage && ./hello-openttd-*.AppImage`

### macOS

1. 解压 zip，将 `hello-openttd.app` 拖入「应用程序」。
...
| Linux | `~/.local/share/hello-openttd`（遵循 XDG） |
| macOS | `~/Library/Application Support/hello-openttd` |

**便携模式**：在启动器程序所在目录放置名为 `hello-openttdData` 的文件夹（或空标记文件 `portable.flag`），启动器将使用该目录作为数据根目录，适合放在 U 盘或同步盘中。
